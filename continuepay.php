...
<?php
    ...
    if (mysqli_connect_errno())
      ...
      $qry=mysqli_query($conn,"select * from payment WHERE booking_id='$bookingId'");
      $data=mysqli_fetch_array($qry);

     $dif=($data['totAmount'])-($data['balAmount']);
     ...
     if(isset($_POST['process']))
  {
    ...
    if($paid > $dif)
    {
        ...
        echo '<script>alert("Amount should be less or equal to the outstanding balance");window.location = "invoice.php";</script>';

        exit;

    }else if($paid==$dif){

   $sum=$paid+($data['balAmount']);
   
 $command="
    UPDATE payment
    SET payment_type='$paytype',payment_method='$paymethod', balAmount='$sum'
    ...
    echo '<script>alert("Remaining amount fully paid.");window.location = "invoice.php";</script>';
    ...
    }elseif($paid < $dif)
    {
        ...
        $command="
           ...
           echo '<script>alert("Deposit amount paid.");window.location = "invoice.php";</script>';
...
var paymerror=document.getElementById("paymerror");
...
paymerror.innerHTML="<span style='color:red;''>"+" please select payment method. *</span>"
...
var paym=document.forms["form"]["paymentmethod"].value;

if(paym==""){

    paymerror.innerHTML="<span style='color:red;''>"+" Please select payment type.*</span>"
    return false;
...
<body>
            ...
            <div class="container"><a class="navbar-brand" href="" style="font-family: Asap, sans-serif;font-weight: normal;">SAYFT&nbsp;<i class="fa fa-calendar"></i>&nbsp;</a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
                <div
                    ...
                    <ul class="nav navbar-nav ml-auto" style="color: rgb(255,255,255);">
                        <li class="nav-item" role="presentation"><a class="nav-link" href="invoice.php" style="color: rgb(255,255,255);font-weight: normal;font-family: Asap, sans-serif;">Back</a></li>
    ...
    <div class="row login-form">
        <div class="col-md-4 offset-md-4" style="padding-top: 131px;">
            <h2 class="text-center shadow-sm form-heading" style="font-family: Asap, sans-serif;font-weight: normal;color: rgb(240,62,51);">Payment Remaining Balance&nbsp;<i class="fa fa-money"></i></h2>
            <form class="text-white border-white shadow custom-form" name="form" onsubmit="return validateForm();"  method="post">
                <div class="col">
                    <h1 style="font-size: 16px;font-family: Asap, sans-serif;color: rgb(240,62,51);">Provide Details<strong>(Outstanding Balance :R<?php echo number_format($dif,2) ?>)</strong> </h1>
        ...
        <div class="form-group"><select class="form-control" value="<?php echo $data['payment_method']?>" name="paymentmethod" id="paymentmethod" style="font-family: Asap, sans-serif;">
              
                    <option value="" selected="">Select Payment Method</option>
                <option value="EFT">EFT</option>
              
        
        </select><span id="paymerror"></span></div>

                <div class="form-group"><input class="form-control" readonly="false" value="<?php echo $dif  ?>" name="price" id="price" type="number" min="1.0" placeholder="Amount"></div>
