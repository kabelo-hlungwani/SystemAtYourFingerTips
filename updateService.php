...
$query="SELECT * FROM service WHERE  service.service_id='$idserv';";
...
SET
user_id='$id',service_type='$servicetype' , image='".$upload_file."', description='$description', price='$price'
WHERE service='$idserv'";
...
echo '<script>alert(" service successfully Updated");window.location = "MyAdvetView.php";</script>';
...
else if(!description.match(/^([a-zA-Z0-9 _- ]+$/))
{
    ...
    <div class="row login-form">
        <div class="col-md-4 offset-md-4" style="padding-top: 131px;">
            ...
            <form class="text-white border-white shadow custom-form" action="" name="form"  enctype="multipart/form-data" onsubmit="return validateForm();" method="post">
                <div class="col">
                    <h1 style="font-size: 16px;font-family: Asap, sans-serif;color: rgb(240,62,51);font-style: italic;">Update Service Details</h1>
                    <hr style="background-color: #f03e33;">
                </div>
                <div class="form-group"><select name="servicetype" value="<?php echo $rows['service_type'];?>" id="servicetype"  class="form-control" style="font-family: Asap, sans-serif;">
                    <optgroup label="Service Type">
                       
                        <option disabled selected hidden ><?php echo $rows['service_type'];?></option>
                ...
                <h1
                    ...
                    <br>
                    <div class="form-group"><input class="form-control" name="photo" value="<?php echo $rows['image'];?>" id="photo" accept="image/x-png,image/jpeg,image/jpg" style="font-family: Asap, sans-serif;" type="file" style="font-family: Asap, sans-serif;"><span id="errorpic"></span></div>
                    <div class="form-group"><textarea class="form-control" name="description"  id="description" placeholder="Description"><?php echo $rows['description'];?></textarea><span id="derror"></span></div>
                    <div class="form-group"><input class="form-control" name="price" id="price" value="<?php echo $rows['price'];?>" type="number" placeholder="Booking Fee" min="0.00" max="100000.00" step="0.01"><span id="errorprice"></span></div>
                    <input class="btn btn-light btn-block btn-sm submit-button" type="submit" name="Update" value="Update" style="background-color: rgb(240,62,51);font-weight: normal;font-family: Asap, sans-serif;font-size: 16px;">Update</button>
                    <a
                        href="MyAdvertView.php" style="color: rgb(240,62,51);font-family: Asap, sans-serif;">Cancel</a>
            </form>
        </div>
    </div>
